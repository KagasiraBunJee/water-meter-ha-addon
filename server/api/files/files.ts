...
export const files = (
  app: Application,
  authHandler: (req: Request, res: Response, next: any) => void
) => {
  app.post(
    "/api/devices/:deviceID/attachment/start",
    authHandler,
  ...
  app.put(
    "/api/devices/:deviceID/attachment/append/:fileID",
    authHandler,
    async (req: Request, res: Response) => {
      const deviceID = req.params.deviceID;
      ...
      const foundDevice = await Device.findOne({ deviceID });
      const attachment = await FileAttachment.findOneAndUpdate(
        { fileID },
        { state: "uploading" }
      );
      ...
      try {
        ...
        console.error("Failed to append file data");
  ...
  app.post(
    "/api/devices/:deviceID/attachment/finish/:fileID",
    authHandler,
    async (req: Request, res: Response) => {
      const deviceID = req.params.deviceID;
      const fileID = req.params.fileID;
  ...
  app.get(
    "/api/devices/:deviceID/attachment/latest",
    async (req: Request, res: Response) => {
      console.log("latest");
      const deviceID = req.params.deviceID;
      ...
      let attachment;
      if (fileID) {
        attachment = await FileAttachment.findOne({
          deviceID,
          fileID,
          state: "ready",
        });
  ...
  app.get(
    "/api/devices/attachment/cleanup",
    authHandler,
    async (req: Request, res: Response) => {
